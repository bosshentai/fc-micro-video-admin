...
export class MediaFileValidator {
  constructor(
    ...
    private readonly valid_mime_type: string[],
  ) {}
  ...
  validate({
    ...
    if (!this.validateMimeType(mime_type)) {
      throw new InvalidMediaFileMimeTypeError(mime_type, this.valid_mime_type);
    }
  ...
  private validateMimeType(mime_type: string) {
    return this.valid_mime_type.includes(mime_type);
  }
