...
describe('GenreSequelizeRepository Integration Tests', () => {
  ...
  it('should throw error on delete when a entity not found', async () => {
    ...
    await expect(
      genreRepo.delete(new GenreId('93d0d4c1-1c6a-4b6a-9c9f-1c6a4b6a9c9f')),
    ).rejects.toThrow(
      new NotFoundError('93d0d4c1-1c6a-4b6a-9c9f-1c6a4b6a9c9f', Genre),
    );
  });

  it('shoud delete a entity', async () => {
    const category = Category.fake().aCategory().build();
    await categoryRepo.insert(category);
    const genre = Genre.fake()
      .aGenre()
    ...
    const entityFound = await GenreModel.findByPk(genre.genre_id.id);
  ...
  describe('search method tests', () => {
    ...
    it('should apply paginate and filter bt categories_id', async () => {
