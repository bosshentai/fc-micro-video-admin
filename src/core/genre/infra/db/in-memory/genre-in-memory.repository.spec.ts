...
describe('GenreInMemoryRepository', () => {
  ...
  it('should sort by created_at when sort param is null', async () => {
    ...
    const itemsSortted = await repository['applySort'](items, null, null);
    expect(itemsSortted).toStrictEqual([items[2], items[1], items[0]]);
  });
  ...
  it('should sort by name', async () => {
    ...
    const itemsSortted = await repository['applySort'](items, 'name', 'asc');
    expect(itemsSortted).toStrictEqual([items[2], items[1], items[0]]);

    const itemsSorttedDesc = await repository['applySort'](
      items,
      'name',
      'desc',
    );
    expect(itemsSorttedDesc).toStrictEqual([items[0], items[1], items[2]]);
  });
});
