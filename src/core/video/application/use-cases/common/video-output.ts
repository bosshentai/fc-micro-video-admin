  ...
  static toOutput({
    ...
    return {
      ...
      categories: allCategoriesOfVideoAndGenre
        .filter((category) => video.categories_id.has(category.category_id.id))
        .map((category) => ({
          id: category.category_id.id,
          name: category.name,
          created_at: category.created_at,
        })),
      genres_id: Array.from(video.genres_id.values()).map((id) => id.id),
      ...
      cast_members: cast_members
        .filter((castMember) =>
          video.cast_members_id.has(castMember.cast_member_id.id),
        )
        .map((castMember) => ({
