...
export class VideoSequelizeRepository implements IVideoRepository {
  ...
  async bulkInsert(entities: Video[]): Promise<void> {
    const model = entities.map((e) => VideoModelMapper.toModelProps(e));
    await this.videoModel.bulkCreate(model, {
      include: this.relations_include,
      transaction: this.uow.getTransaction(),
    });
  ...
  async findById(entity_id: VideoId): Promise<Video | null> {
  ...
  async existsById(
    ...
    const existsVideoIds = existsVideoModels.map(
      (model) => new VideoId(model.video_id),
    );

    const notExistsVideoIds = ids.filter(
      (id) => !existsVideoIds.some((videoId) => videoId.equals(id)),
    );
  ...
  async update(entity: Video): Promise<void> {
    ...
    await Promise.all([
      ...model.image_medias.map((image) =>
        image.destroy({ transaction: this.uow.getTransaction() }),
      ),
      ...model.audio_video_medias.map((audio) =>
        audio.destroy({ transaction: this.uow.getTransaction() }),
      ),
      model.$remove(
        'categories',
      ...
      model.$remove(
        'genres',
        model.genres_id.map((genre) => genre.genre_id),
        {
          transaction: this.uow.getTransaction(),
        },
      ),
      model.$remove(
        'cast_members',
        model.cast_members_id.map((cast_member) => cast_member.cast_member_id),
        {
          transaction: this.uow.getTransaction(),
        },
      ),
