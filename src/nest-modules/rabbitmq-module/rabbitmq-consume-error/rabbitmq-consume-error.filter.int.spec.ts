...
describe('RabbitmqConsumeErrorFilter Integration tests', () => {
  ...
  it('should not retry if error is non-retriable', async () => {
    ...
    const spyHandleMessage = jest.spyOn(
      amqpConnection,
      'deserializeMessage' as any,
    );
    ...
    // console.log(spyHandleMessage.mock.results[0].value);

    // console.log(spyHandleMessage.mock.results[0].value instanceof Promise);

    expect(spyHandleMessage.mock.results[0].value).toEqual({
      message: 'test',
      headers: {},
    });
  ...
  it('should retry if error is retriable and retry count is less than max retries', async () => {
    ...
    const spyPublish = jest.spyOn(amqpConnection, 'publish');
