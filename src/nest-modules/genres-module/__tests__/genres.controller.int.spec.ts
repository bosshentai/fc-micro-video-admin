...
describe('GenresController Integration Tests', () => {
  ...
  describe('should update a category', () => {
    ...
    test.each(arrange)(
      ...
      async ({ entity: genre, send_data, expected, relations }) => {
        ...
        const categoriesOfGenre = relations.categories.filter((category) =>
          genreUpdated!.categories_id.has(category.category_id.id),
        );
  ...
  describe('search method', () => {
    describe('should returns categories using query empty ordered bt created_at', () => {
      ...
      test.each(arrange)(
        ...
        async ({ send_data, expected }) => {
          ...
          const expectedPresenter = new GenreCollectionPresenter({
            items: entities.map((genre) => ({
              ...genre.toJSON(),
              id: genre.genre_id.id,
              categories_id: expect.arrayContaining(
                Array.from(genre.categories_id.keys()),
              ),
              categories: Array.from(genre.categories_id.keys()).map((id) => ({
